// 
#include "../extern/mxml/mxml.h"
#include "game_patch_xml_cfg.hpp"
#include <stdint.h>
#include <stdio.h>  // Added for standard IO functions like FILE, fopen, printf etc.
#include <stdlib.h> // Added for malloc, free
#include <string.h> // Added for strncmp, __builtin_strlen (though <cstring> is preferred in C++)

#include "game_patch_memory.hpp"
#include "notify.hpp"

// Assuming startsWith is defined elsewhere, perhaps like this:
// #include <stdbool.h> // For bool type if not in C++ context, but it seems C++
inline bool startsWith(const char *str, const char *prefix) {
    return strncmp(str, prefix, strlen(prefix)) == 0;
}


// Include the `game_patch_fliprate_list.xml` as a symbol
__asm__(
    ".intel_syntax noprefix\n"
    ".section .data\n"
    "DefaultXml_FliprateList:\n"
    ".incbin \".." XML_PATH_LIST "\"\n");

extern "C" const char DefaultXml_FliprateList[];

int makeDefaultXml_List()
{
    FILE *f = fopen(XML_PATH_LIST, "rb");
    if (!f)
    {
        FILE *new_f = fopen(XML_PATH_LIST, "w");
        if (!new_f) { // Add error check for fopen
            perror("Failed to open " XML_PATH_LIST " for writing");
            return -1; // Indicate failure
        }
        // Print default data to TTY
        printf("%s\n", DefaultXml_FliprateList);
        fputs(DefaultXml_FliprateList, new_f);
        fflush(new_f);
        fclose(new_f);
        printf_notification("Created default config file:\n" XML_PATH_LIST);
    } else {
        fclose(f); // Close the file if it was opened for reading
    }
    return 0;
}

// extern "C" void *malloc(size_t); // These are usually included via <stdlib.h>
// extern "C" void free(void *);

int Xml_parseTitleID(const char *titleId)
{
    makeDefaultXml_List();
    char *buffer = NULL;
    uint64_t length = 0;
    FILE *f = fopen(XML_PATH_LIST, "rb");
    // Corrected Line 42
    printf("File: " XML_PATH_LIST " exist at 0x%p\n", (void*)f);
    if (f)
    {
        fseek(f, 0, SEEK_END);
        length = ftell(f);
        fseek(f, 0, SEEK_SET);
        buffer = (char *)malloc(length + 1); // Allocate one extra byte for null terminator
        if (buffer)
        {
            size_t bytes_read = fread(buffer, 1, length, f);
            if (bytes_read != length) {
                 // Handle potential read error, although mxmlLoadString might handle partial reads
                 fprintf(stderr, "Warning: Could not read full file content of %s\n", XML_PATH_LIST);
            }
            buffer[length] = '\0'; // Null-terminate the buffer for string functions
            // Corrected Line 52
            printf("Memory at 0x%p\n", (void*)buffer);
        }
        fclose(f);
    } else {
        perror("Failed to open " XML_PATH_LIST " for reading");
        return 0; // Indicate failure or inability to parse
    }

    // Check if buffer allocation succeeded before using it
    if (!buffer) {
         fprintf(stderr, "Failed to allocate memory for XML buffer\n");
         return 0; // Indicate failure
    }


    mxml_node_t *tree = mxmlLoadString(NULL, buffer, MXML_OPAQUE_CALLBACK);

    if (tree == NULL)
    {
         fprintf(stderr, "Failed to parse XML from %s\n", XML_PATH_LIST);
         // Clean up allocated buffer even if parsing fails
         if (buffer) {
            free(buffer);
         }
         return 0;
    }

    mxml_node_t *titleIDNode = mxmlFindElement(tree, tree, "TitleID", NULL, NULL, MXML_DESCEND);

    int found_id = 0;
    if (titleIDNode != NULL)
    {
        mxml_node_t *idNode = mxmlFindElement(titleIDNode, tree, "ID", NULL, NULL, MXML_DESCEND);

        // Assuming __builtin_strlen works like strlen, otherwise include <string.h> or <cstring>
        size_t titleIdLen = strlen("CUSAxxxxx"); // Use standard strlen

        while (idNode != NULL)
        {
            const char *idValue = mxmlGetOpaque(idNode);
            if (idValue != NULL)
            {
                // printf("TitleID: %s\n", idValue);
                // Use titleIdLen calculated outside the loop
                if (strncmp(titleId, idValue, titleIdLen) == 0)
                {
                    found_id = 1;
                    printf("%s match !! found_id=0x%08x\n", titleId, found_id);
                    break;
                }
            }
            idNode = mxmlFindElement(idNode, titleIDNode, "ID", NULL, NULL, MXML_NO_DESCEND);
        }
    }

    if (buffer)
    {
        free(buffer);
    }
    if (tree)
    {
        mxmlDelete(tree);
    }
    return found_id;
}

int simple_get_bool(const char *val)
{
    if (val == NULL || val[0] == 0)
    {
        return 0;
    }
    if (
        val[0] == '1' ||
        startsWith(val, "on") ||
        startsWith(val, "true") ||
        startsWith(val, "On") ||
        startsWith(val, "True"))
    {
        return 1;
    }
    else if (
        val[0] == '0' ||
        startsWith(val, "off") ||
        startsWith(val, "false") ||
        startsWith(val, "Off") ||
        startsWith(val, "False"))
    {
        return 0;
    }
    return 0; // Default case if none match
}

// This is so bad but we want to generate a default config with the same key
// that is used for comparisons
#define SET_XML_KEY(key, value) "    <" key ">" #value "</" key ">\n"
const char DefaultCfgData[] = ""
                              "<cfg>\n"
                              "    \n"
                                SET_XML_KEY(GR1_EN, 0)
                                SET_XML_KEY(GR2_60HzKey, 0)
                                SET_XML_KEY(BB_60FPSKey, 1)
                                SET_XML_KEY(BB_MBKey, 0)
                                SET_XML_KEY(BB_CAKey, 0)
                                SET_XML_KEY(BB_DebugCameraKey, 0)
                                SET_XML_KEY(BB_ColorBorder, 0)
                                SET_XML_KEY(BB_Vsync, 0)
                                SET_XML_KEY(BB_1280_720, 0)
                                SET_XML_KEY(SOTC_DebugMenu, 0)
                                SET_XML_KEY(TO1886_60FPS, 1)
                                SET_XML_KEY(TO1886_16_9, 0)
                                SET_XML_KEY(Big4R_MainDebugMenu, 0)
                                SET_XML_KEY(Big4R_TLLDebugMenu, 0)
                                SET_XML_KEY(DemonSouls_UnlockFPS, 0)
                                SET_XML_KEY(DemonSouls_DebugMenu, 0)
                                SET_XML_KEY(DemonSouls104_DebugMenu, 0)
                                SET_XML_KEY(Driveclub_60FPS, 1)
                                SET_XML_KEY(Driveclub_DLC_Unlock, 1)
                                SET_XML_KEY(TLG_60FPS, 0)
                                SET_XML_KEY(t2ps4_109_1080p_in120Hz, 0)
                                SET_XML_KEY(BPR_103_120Hz, 0)
                                SET_XML_KEY(BPR_103_1080p, 1)
                                SET_XML_KEY(BPR_103_SkipLogos, 1)
                                SET_XML_KEY(BPR_103_SkipIntroVideo, 0)
                                SET_XML_KEY(UCC_DebugMenu, 0)
                                "</cfg>\n";
#undef SET_XML_KEY

int makeDefaultXml_Cfg()
{
    FILE *f = fopen(XML_PATH, "rb");
    if (!f)
    {
        FILE *new_f = fopen(XML_PATH, "w");
         if (!new_f) { // Add error check for fopen
            perror("Failed to open " XML_PATH " for writing");
            return -1; // Indicate failure
        }
        // Print the default data to TTY
        printf("%s\n", DefaultCfgData);
        fputs(DefaultCfgData, new_f);
        fflush(new_f);
        fclose(new_f);
        printf_notification("Created default config file:\n" XML_PATH);
    } else {
         fclose(f); // Close the file if it was opened for reading
    }
    return 0;
}

int parseXML(const char *xml_key)
{
    makeDefaultXml_Cfg();
    char *buffer = NULL;
    uint64_t length = 0;
    FILE *f = fopen(XML_PATH, "rb");
    int ret = 0; // Default return value if key isn't found or parsing fails

    // Corrected Line 186
    printf("File " XML_PATH " exist at 0x%p\n", (void*)f);
    if (f)
    {
        fseek(f, 0, SEEK_END);
        length = ftell(f);
        fseek(f, 0, SEEK_SET);
        buffer = (char *)malloc(length + 1); // Allocate one extra byte for null terminator
        if (buffer)
        {
            size_t bytes_read = fread(buffer, 1, length, f);
             if (bytes_read != length) {
                 // Handle potential read error
                 fprintf(stderr, "Warning: Could not read full file content of %s\n", XML_PATH);
             }
            buffer[length] = '\0'; // Null-terminate the buffer
            // Corrected Line 196
            printf("Memory at 0x%p\n", (void*)buffer);
        }
        fclose(f);
    }
    else
    {
        perror("Failed to open " XML_PATH " for reading");
        return -2; // Indicate file open error specifically
    }

    // Check if buffer allocation succeeded before using it
    if (!buffer) {
         fprintf(stderr, "Failed to allocate memory for XML buffer\n");
         return -1; // Indicate memory allocation failure
    }

    mxml_node_t *tree = mxmlLoadString(NULL, buffer, MXML_OPAQUE_CALLBACK);
    if (tree == NULL)
    {
        printf("Failed to parse XML from %s.\n", XML_PATH);
        // Clean up allocated buffer even if parsing fails
        if (buffer) {
           free(buffer);
        }
        return 0; // Return default value on parse failure
    }

    mxml_node_t *xml_element = mxmlFindElement(tree, tree, xml_key, NULL, NULL, MXML_DESCEND);
    if (xml_element)
    {
        const char *value = mxmlGetOpaque(xml_element);
        if (value)
        {
            ret = simple_get_bool(value);
            printf("%s: (%s) 0x%08x\n", xml_key, value, ret);
        }
        // If value is NULL or element not found, ret remains 0 (the default)
    }

    if (buffer)
    {
        free(buffer);
    }
    if (tree)
    {
        mxmlDelete(tree);
    }
    return ret;
}